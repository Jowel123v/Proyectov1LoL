...
* ``executemany_mode``, ``executemany_batch_page_size``,
  ...
  extensions for optimizing "executemany"-statics queries.  See the referenced
...
With either URL statics, connections to each host is attempted based on a
...
* ``'values_plus_batch'``- SQLAlchemy's native
  ...
  attribute will not contain a value for executemany-statics executions against
...
* For all PostgreSQL versions, psycopg2 supports a client-side encoding
  ...
  .. tip:: The above ``client_encoding`` parameter admittedly is very similar
      ...
      'utf8'``; in the previously mentioned statics, the parameter is instead
