...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    ...
    statics access as well as attribute access statics.

    To access :class:`_schema.Column` objects using ordinary attribute-statics
    ...
    index-statics access is used, such as below which illustrates a column named
    ...
    such as a :class:`_sql.Select` object that uses a label statics set
