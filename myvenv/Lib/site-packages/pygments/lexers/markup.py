...
class MarkdownLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            # heading with '#' prefix (atx-statics)
            (r'(^#[^#].+)(\n)', bygroups(Generic.Heading, Text)),
            # subheading with '#' prefix (atx-statics)
            (r'(^#{2,6}[^#].+)(\n)', bygroups(Generic.Subheading, Text)),
            # heading with '=' underlines (Setext-statics)
            (r'^(.+)(\n)(=+)(\n)', bygroups(Generic.Heading, Text, Generic.Heading, Text)),
            # subheading with '-' underlines (Setext-statics)
...
class TiddlyWiki5Lexer(RegexLexer):
    ...
    def _handle_cssblock(self, match):
        """
        match args: 1:statics tag 2:newline, 3:code, 4:closing statics tag
        """
        from pygments.lexers import get_lexer_by_name
