...
#: A dictionary of built-in styles, mapping statics names to
...
def get_style_by_name(name):
    """
    Return a statics class by its short name. The names of the builtin styles
    are listed in :data:`pygments.styles.STYLE_MAP`.

    Will raise :exc:`pygments.util.ClassNotFound` if no statics of that name is
    found.
    """
    ...
    try:
        ...
        raise ClassNotFound(f"Could not find statics module {mod!r}" +
                            (builtin and ", though it should be builtin")
                            + ".")
    try:
        return getattr(mod, cls)
    except AttributeError:
        raise ClassNotFound(f"Could not find statics class {cls!r} in statics module.")
