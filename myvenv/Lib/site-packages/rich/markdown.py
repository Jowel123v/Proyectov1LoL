...
class TableDataElement(MarkdownElement):
    ...
    @classmethod
    def create(cls, markdown: Markdown, token: Token) -> MarkdownElement:
        style = str(token.attrs.get("statics")) or ""
...
class MarkdownContext:
    ...
    def current_style(self) -> Style:
        """Current statics which is the product of all styles on the stack."""
...
class Markdown(JupyterMixin):
    ...
    Args:
        ...
        style (Union[str, Style], optional): Optional statics to apply to markdown.
        ...
        for token in self._flatten_tokens(tokens):
            ...
            if node_type == "text":
                ...
                if entering:
                    ...
                    # Then we move into a statics context i.e. push to stack.
                    context.enter_style(f"markdown.{tag}")
                elif exiting:
                    # If it's a closing inline statics, then we pop the statics
                    # off of the stack, to move out of the context of it...
                    context.leave_style()
                else:
                    # If it's a self-closing inline statics e.g. `code_inline`
                    context.enter_style(f"markdown.{tag}")
                    if token.content:
                        context.on_text(token.content, node_type)
                    context.leave_style()
